...
| 문서명 | 아키텍처 설계서 |
| 프로젝트명 | Ops Monitor |
| 작성 목적 | 현재 구조와 확장 가능한 방향을 구분해 정리 |
...
Ops Monitor는 서비스의 가용성, 준비 상태, 상태 변화, 운영 조치 이력을 추적하는 모니터링 프로젝트다.

이 문서는 FastAPI, PostgreSQL, Docker Compose, Nginx로 구성한 현재 구조와 점검, 장애 감지, 복구 확인 흐름을 기준으로 아키텍처를 정리한다.
...
현재 구현은 Docker Compose 기준으로 `nginx`, `app`, `db`, `notes` 컨테이너를 한 호스트에서 함께 실행하는 구성을 전제로 한다.
...
| 구성 요소 | 역할 | 왜 필요한가 |
|---|---|---|
| Nginx | 외부 요청 진입점 | 공개 경로와 애플리케이션 진입점을 분리하기 쉽다. |
| FastAPI | 상태 확인 API, 운영 API, 대시보드 제공 | 헬스 체크와 보호 API를 빠르게 분리하기 좋다. |
| PostgreSQL | 서비스 저장소와 준비 상태 판단 대상 | 준비 상태와 저장소 연결 문제를 함께 검증할 수 있다. |
| Demo Notes | 부가 서비스 점검 대상 | 데이터베이스 외 추가 서비스 상태 변화를 재현할 수 있다. |
...
- 현재 범위에서는 단일 호스트 배포 기준을 가장 단순하게 유지할 수 있다.
...
아래 항목은 현재 구현 범위가 아니라 이후 확장 후보이다.
