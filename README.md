<div align="center">
  <h1>Ops Monitor</h1>
  <p>WEB/WAS 운영 관점에서 상태 점검, 실행 기록, 운영 액션 추적을 구현한 경량 모니터링 프로젝트</p>
...
Ops Monitor는 서비스 운영자가 여러 화면을 오가며 확인하던 생존 신호, 준비 상태, 데이터베이스 연결, 부가 서비스 상태, 시스템 자원 사용량, 최근 장애 이벤트를 한 흐름으로 묶어본 프로젝트입니다.

단순히 상태값을 나열하는 화면보다, 운영자가 실제로 판단해야 하는 질문에 답할 수 있는 구조를 만드는 데 초점을 맞췄습니다.

포트폴리오 관점에서는 WEB/WAS 운영자가 무엇을 기준으로 상태를 해석하고, 장애를 어떻게 기록하며, 수동 조치를 어떤 형태로 추적하는지를 코드와 문서로 설명하는 데 의미를 둡니다.
