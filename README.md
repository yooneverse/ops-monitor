...
최근 변경으로 런타임 설정 안정성과 대시보드 가시성을 보강했습니다.

- 잘못된 모니터링 설정값이 들어와도 앱이 안전한 기본값으로 동작
- `/monitoring/status`에서 threshold, auth 설정 여부, config warning 확인 가능
- `/dashboard`에서 자동 새로고침 주기, threshold 정책, 설정 경고를 함께 표시
- 관련 운영 가이드를 별도 문서로 정리

자세한 내용은 [docs/10_runtime_configuration.md](docs/10_runtime_configuration.md)에서 확인할 수 있습니다.
...
1. `GET /monitoring/status`에서 interval, thresholds, `config_warnings` 확인
2. `/dashboard`에서 `Config Status`와 `Configuration Warnings` 확인
3. 보호된 API가 Basic Auth로 정상 보호되는지 확인
